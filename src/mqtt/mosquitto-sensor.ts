...
export async function updateSensorState (userName: string, houseName: string, sensor: string, message: string) {
   try {
      ...
      await sensorDataAccess.updateState(userName, houseName, sensorNumber, state);
   } catch (err: any) {
      console.log(`Error (method: "updateSensorState"): ${err.message as string}`);
   }
}

export async function updateSensorHistory (userName: string, houseName: string, message: string) {
   try {
      const sensorNumber = verifySensorNumber(message);

      const activationDate = new Date(Date.now());

      await sensorDataAccess.addActivationDate(userName, houseName, sensorNumber, activationDate);
   } catch (err: any) {
